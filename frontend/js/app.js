...
- Maiores despesas do mês: ${topExp||'nenhuma'}`;
    ...
    // Strip any full HTML page tags that would break the layout
    let aiText = data.text || '';
    aiText = aiText.replace(/```html/gi, '').replace(/```/g, '');
    aiText = aiText.replace(/<(!DOCTYPE|html|head|body|style|script)[^>]*>[\s\S]*?<\/(html|head|body|style|script)>/gi, '');
    aiText = aiText.replace(/<(!DOCTYPE|html|head|body)[^>]*>/gi, '');
    aiText = aiText.replace(/<style[\s\S]*?<\/style>/gi, '');
    aiText = aiText.replace(/<script[\s\S]*?<\/script>/gi, '');
    aiText = aiText.trim();

    el('ia-loading').classList.add('hidden');
    el('ia-result').classList.remove('hidden');
    el('ia-result-body').innerHTML = aiText;

...
router.post('/register', async (req, res) => {
  try {
    ...
    if (!name || !email || !password || !invite_code)
      return res.status(400).json({ error: 'Preencha todos os campos, incluindo o código de acesso.' });
    ...
    if (codeRows[0].used_by)
      return res.status(403).json({ error: 'Este código já foi utilizado.' });
